...
#Compares DEG in each population to the rest of the corresponding object

Idents(obj) <- "FinalName"
DimPlot(obj)

## find markers per cell type instead of cluster

markers_celltypes <- FindAllMarkers(obj, min.pct = 0.25, only.pos = T) ## only positive 
write.csv(markers_celltypes, file = "markers_celltypes.csv")
...
## After subsetting neurons, find markers in different neuronal populations 
markers_neurons <- FindAllMarkers(neurons, min.pct = 0.25, only.pos = T) ## only positive 
write.csv(markers_neurons, file = "markers_neurons.csv")

#Compare CPN to the rest, ie make CPN and other neurons. Name all the other neurons 'other'
...
write.csv(markers_neurons_CPN, file = "markers_neurons_CPN_allorg.csv")
...
write.csv(markers_neurons_CFuPN, file = "markers_neurons_CFuPN_allorg.csv")
...
write.csv(markers_neurons_PN, file = "markers_neurons_PN_allorg.csv")
...
## After subsetting neurons, find markers in different neuronal populations 
markers_progs <- FindAllMarkers(progs, min.pct = 0.25, only.pos = T) ## only positive 
write.csv(markers_progs, file = "markers_progs.csv")
...
write.csv(markers_progs_aRG, file = "markers_progs_aRG_allorg.csv")
...
write.csv(markers_progs_IP, file = "markers_progs_IP_allorg.csv")
...
write.csv(markers_progs_oRG, file = "markers_progs_oRG_allorg.csv")
